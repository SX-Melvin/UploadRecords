...
var emailFrom = Configuration.GetRequiredValue(config, "Email:From"); // Email Sender
var emailHost = Configuration.GetRequiredValue(config, "Email:Host"); // Email Host
var emailPort = Int32.Parse(Configuration.GetRequiredValue(config, "Email:Port")); // Email Port
var dbConnectionStr = Configuration.GetRequiredValue(config, "Database:ConnectionString"); // Db Connection
var logPath = Configuration.GetRequiredValue(config, "Audit:Path"); // Where we store the success and fail logs
var batchFolder = Configuration.GetRequiredValue(config, "Batch:FolderPath"); // Where the batch folder located
...
long batchUploadNodeID = long.Parse(Configuration.GetRequiredValue(config, "Batch:UploadFolderNodeID")); // Where to upload the files / folder
List<long> functionalAdminIDs = config.GetSection("FunctionalAdminID").Get<List<long>>() ?? throw new InvalidOperationException("FunctionalAdminID configuration is missing."); // Functional admin IDs
...
Logger.Information("OTCS Secret: " + new string('*', otcsSecret.Length));
Logger.Information("OTCS ApiUrl: " + otcsApiUrl);
Logger.Information("NodeID: " + batchUploadNodeID);
...
try
{
    ...
    for (int i = 0; i < metadatas.Count; i++)
    {
        ...
        var scanner = new Scanner(batchFolder, logPath, otcs, metadata, division, batchUploadNodeID, functionalAdminIDs);
    ...
    if(metadatas.Count > 0)
    {
        ...
        {
            OTCS = otcs,
            ReportNodeLocationID = long.Parse(Configuration.GetRequiredValue(config, "OTCS:ReportNodeLocationID")),
